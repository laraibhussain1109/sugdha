...
def generate_checksum(param_dict, merchant_key, salt=None):
    ...
    return __encode__(hash_string, IV, __extend_key__(merchant_key))
...
def verify_checksum(param_dict, merchant_key, checksum):
    ...
    paytm_hash = __decode__(checksum, IV, __extend_key__(merchant_key))
    salt = paytm_hash[-4:]
...
def __extend_key__(key):
    return key.ljust(KEY_LENGTH)[:KEY_LENGTH]
